...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    if (Math.abs(x2 - x1) === 1 && Math.abs(y2 - y1) === 1) { return Math.PI/2}
    if (x2 - x1==0 && Math.abs(y2 - y1) === 2 ) { return Math.PI}
    if (y2 - y1==0 && Math.abs(x2 - x1) === 2 ) { return Math.PI}
    return 0
}
...
function parseNumberFromString(value) {
    ...
    if (Number.isInteger(a)) { return Math.round(a) }
   return a
}
